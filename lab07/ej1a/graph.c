...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    cost_t *costs_array= calloc(max_vertexs*max_vertexs, sizeof(cost_t)); //ARREGLO DINAMICO 1D
    
    //Reservamo un bloque de memoria(max_vertexs*max_vertexs) y luego accedemos a los elementos de manera lineal,
    //Representamos un matriz bidimensional como un vector.
